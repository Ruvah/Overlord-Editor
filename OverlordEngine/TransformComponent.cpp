...
void TransformComponent::Serialize(YAML::Emitter & out)
{
	...
	out << YAML::BeginMap;
	out << YAML::Key << "Type";
	out << YAML::Value << "TransformComponent";
	SERIALIZE_VAR(m_Position, out);
	SERIALIZE_VAR(m_Scale, out);
	SERIALIZE_VAR(m_Rotation, out);

	out << YAML::EndMap;

}

void TransformComponent::ParseFromNode(YAML::Node & Info)
{
	PARSE_VAR(m_Position, Info);
	PARSE_VAR(m_Scale, Info);
	PARSE_VAR(m_Rotation, Info);
...
const DirectX::XMFLOAT3 TransformComponent::GetRotationEuler() const
{
	DirectX::XMFLOAT3 euler{};
	OVLFunc::DXQuatToEuler(m_Rotation,euler);
	return euler;
}
