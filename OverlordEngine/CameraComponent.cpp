...
void CameraComponent::Serialize(YAML::Emitter & out)
{
	...
	out << YAML::BeginMap;
	out << YAML::Key << "Type";
	out << YAML::Value << "CameraComponent";
	SERIALIZE_VAR(m_FarPlane,out);
	SERIALIZE_VAR(m_FOV, out);
	SERIALIZE_VAR(m_NearPlane, out);
	SERIALIZE_VAR(m_Size, out);
	SERIALIZE_VAR(m_IsActive, out);
	SERIALIZE_VAR(m_PerspectiveProjection, out);

	out << YAML::EndMap;
	
}

void CameraComponent::ParseFromNode(YAML::Node & info)
{
	PARSE_VAR(m_FarPlane,info);
	PARSE_VAR(m_FOV,info);
	PARSE_VAR(m_IsActive,info);
	PARSE_VAR(m_NearPlane,info);
	PARSE_VAR(m_PerspectiveProjection,info);
	PARSE_VAR(m_Size,info);


}

physx::PxVec3 CameraComponent::GetRayDir()
{
	using namespace DirectX;
