...
void RigidBodyComponent::Serialize(YAML::Emitter & out)
{
	...
	out << YAML::BeginMap;
	out << YAML::Key << "Type";
	out << YAML::Value << "RigidBodyComponent";
	SERIALIZE_VAR(m_isKinematic,out);
	SERIALIZE_VAR(m_isStatic, out);
	SERIALIZE_VAR(m_InitialConstraints, out);
	SERIALIZE_VAR(m_CollisionGroups, out);
	
	out << YAML::EndMap;

}

void RigidBodyComponent::ParseFromNode(YAML::Node & Info)
{
	PARSE_VAR(m_isKinematic, Info);
	PARSE_VAR(m_isStatic, Info);
	PARSE_VAR(m_InitialConstraints, Info);
	PARSE_VAR(m_CollisionGroups, Info);
}
